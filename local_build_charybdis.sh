#!/usr/bin/env bash
# Build script for charybdis_esb — Damex ESB dongle setup.
# Uses the west workspace rooted in this repository.
#
# Usage:
#   ./local_build_charybdis.sh [artifact-name]   # build one target
#   ./local_build_charybdis.sh                   # build all targets
#   ./local_build_charybdis.sh --fresh <name>    # force cmake reconfigure
...
for t in targets:
    ...
    # only kesb targets
    if "charybdis_esb" not in shield and "charybdis_esb" not in name:
        continue
...
if filter_name and not results:
    print(f"ERROR: artifact-name '{filter_name}' not found in build.yaml (charybdis targets only)", file=sys.stderr)
    sys.exit(1)
...
build_target() {
    ...
    local build_dir="$WORKSPACE/build_charybdis/$name"
...
if [[ ${#failed[@]} -gt 0 ]]; then
    ...
    echo "All charybdis_esb builds succeeded."
fi
