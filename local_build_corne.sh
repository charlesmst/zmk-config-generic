#!/usr/bin/env bash
# Build script for corne_esb — Damex ESB dongle setup.
# Uses the west workspace rooted in this repository.
#
# Usage:
#   ./local_build_corne.sh [artifact-name]   # build one target
#   ./local_build_corne.sh                   # build all targets
#   ./local_build_corne.sh --fresh <name>    # force cmake reconfigure
...
for t in targets:
    ...
    # only corne_esb targets
    if "corne_esb" not in shield and "corne_esb" not in name:
        continue
...
if filter_name and not results:
    print(f"ERROR: artifact-name '{filter_name}' not found in build.yaml (corne targets only)", file=sys.stderr)
    sys.exit(1)
...
build_target() {
    ...
    local build_dir="$WORKSPACE/build_corne/$name"
...
if [[ ${#failed[@]} -gt 0 ]]; then
    ...
    echo "All corne_esb builds succeeded."
fi
